...
import { AccountTransaction, CredentialSignature, AccountTransactionSignature } from './types';
...
export function buildBasicAccountSinger(privateKey: string): AccountSigner {
    return {
        getSignatureCount() {
            return 1n
        },
...
export function buildAdvancedAccountSigner(keyStructure: AccountKeyStructure) {
    const signatureCount = countSignatures(keyStructure);
    return {
        getSignatureCount() {
            return signatureCount;
        },
        async sign(digest: Buffer, keyStructure: AccountKeyStructure) {
            ...
            for (const [credIndex, credStructure] of Object.entries(keyStructure)) {
...
export function signTransaction(transaction: AccountTransaction, signer: AccountSigner): Promise<AccountTransactionSignature> {
