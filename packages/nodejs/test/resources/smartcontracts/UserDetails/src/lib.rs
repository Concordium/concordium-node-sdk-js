...
#[init(contract = "UserDetails", parameter = "InitParameter")]
fn dcb_init(_ctx: &impl HasInitContext) -> InitResult<DCBBankState> {
    let tokens: InitParameter = _ctx.parameter_cursor().get()?;
    if tokens.age > 10 {
...
#[receive(contract = "UserDetails", name = "insertAmount", payable, parameter = "u8")]
...
#[receive(contract = "UserDetails", name = "smashAmount")]
