...
import type { WalletOption, DropdownConfig } from '../types';
...
export class Dropdown {
  ...
  constructor(container: HTMLElement, options: DropdownConfig = {}) {
    ...
    this.elements = {
      ...
      selectedIcon: container.querySelector(
        DROPDOWN_SELECTORS.SELECTED_ICON
      ) as HTMLImageElement | null,
      radioButtons: container.querySelectorAll(
        DROPDOWN_SELECTORS.RADIO_BUTTONS
      ),
...
export function createDropdownHTML(
  ...
  const optionsHTML = options
    .map((option, index) =>
      createOptionHTML(option, index, options.length, config)
    )
    .join('');
