import { ConsensusStatus } from '../../src/index.js';
...
test('retrieves the consensus status from the node with correct types', async () => {
    ...
    return Promise.all([
        expect(typeof consensusStatus.finalizationCount === 'bigint').toBeTruthy(),
        expect(typeof consensusStatus.blocksVerifiedCount === 'bigint').toBeTruthy(),
        expect(typeof consensusStatus.blocksReceivedCount === 'bigint').toBeTruthy(),
        ...
        expect(Number.isNaN(consensusStatus.blockArriveLatencyEMSD)).toBeFalsy(),
        expect(Number.isNaN(consensusStatus.blockArriveLatencyEMA)).toBeFalsy(),
        expect(Number.isNaN(consensusStatus.blockReceiveLatencyEMSD)).toBeFalsy(),
        expect(Number.isNaN(consensusStatus.blockReceiveLatencyEMA)).toBeFalsy(),
        expect(Number.isNaN(consensusStatus.transactionsPerBlockEMSD)).toBeFalsy(),
        expect(Number.isNaN(consensusStatus.transactionsPerBlockEMA)).toBeFalsy(),

        expect(Number.isNaN(consensusStatus.blockReceivePeriodEMA)).toBeFalsy(),
        expect(Number.isNaN(consensusStatus.blockReceivePeriodEMSD)).toBeFalsy(),
        ...
        expect(Number.isNaN(consensusStatus.finalizationPeriodEMSD)).toBeFalsy(),
        ...
        expect(typeof consensusStatus.lastFinalizedBlockHeight === 'bigint').toBeTruthy(),
        expect(typeof consensusStatus.protocolVersion === 'bigint').toBeTruthy(),
    ]);
});
