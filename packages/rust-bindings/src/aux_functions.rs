...
use encrypted_transfers::types::{
    AggregatedDecryptedAmount, EncryptedAmount, EncryptedAmountAggIndex,
};
use hex;
use rand::thread_rng;
...
pub fn create_encrypted_transfer_aux(input: &str) -> Result<String> {
    let v: SerdeValue = from_str(input)?;
    ...
    let input_amount = incoming_amounts.iter().fold(self_amount, |acc, amount| {
        encrypted_transfers::aggregate(&acc, amount)
    });
    let m = 1 << 16;
    let table = BabyStepGiantStep::new(global_context.encryption_in_exponent_generator(), m);

    let decrypted_amount =
        encrypted_transfers::decrypt_amount::<ExampleCurve>(&table, &secret_key, &input_amount);
