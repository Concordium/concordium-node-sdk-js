...
export async function detectInstalledApps(): Promise<AppDetectionResult> {
  ...
  if (cacheTimestamp && now - parseInt(cacheTimestamp) < 5 * 60 * 1000) {
    ...
    try {
      ...
      result.concordiumIDInstalled = await tryOpenApp(
        'concordiumidapp://',
        500
      );
      ...
      localStorage.setItem(
        'concordiumWalletInstalled',
        result.concordiumWalletInstalled.toString()
      );
      localStorage.setItem(
        'concordiumIDInstalled',
        result.concordiumIDInstalled.toString()
      );
  ...
  if (result.installedApps.length === 0) {
    ...
    result.recommendedAction =
      result.installedApps[0] === 'concordium-wallet'
        ? 'open-wallet'
        : 'open-id';
  } else {
...
export function openAppStore(
  appType: 'concordium-wallet' | 'concordium-id' = 'concordium-wallet'
): void {
  const isIOS =
    /iPad|iPhone|iPod/.test(navigator.userAgent) && !(window as any).MSStream;
  ...
  if (appType === 'concordium-wallet') {
    ...
    if (isIOS) {
      ...
      storeUrl =
        'https://play.google.com/store/apps/details?id=com.concordium.mobile_wallet_testnet';
    } else {
      console.warn('Unsupported platform for app store');
      return;
    }
...
export function openDeepLink(
  appType: 'concordium-wallet' | 'concordium-id',
