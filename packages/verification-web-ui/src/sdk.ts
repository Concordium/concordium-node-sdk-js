...
import { getConfig, setConfig } from '@/config.state';
import { ModalConstants } from '@/constants/modal.constants';
import { ServiceFactory } from '@/services';
import type { PresentationRequest, PresentationResponse, SDKWalletConnectConfig, SessionApprovedEvent } from '@/types';
...
export class ConcordiumVerificationWebUI {
    ...
    static async showProcessingModal(): Promise<void> {
        const { showProcessingModal } = await import('./components/desktop/processing');
        showProcessingModal();
    }
    ...
    static closeModal(): void {
        const existingModal = document.querySelector('.mobile--modal-overlay, .desktop--modal-overlay');
        if (existingModal) {
    ...
    constructor(config?: Record<string, any>) {
        ...
        if (config) {
            ...
            const projectId = (config as any).projectId || (config as any).walletConnect?.projectId;
            const metadata = (config as any).metadata || (config as any).walletConnect?.metadata;
            // Support both walletConnectURI and walletConnectUri for better DX
            const walletConnectURI = (config as any).walletConnectURI || (config as any).walletConnectUri;
     ...
     * ```
     */
    async initWalletConnect(config: SDKWalletConnectConfig, onClose?: () => void): Promise<void> {
        if (!config.projectId) {
        ...
        if (!network) {
            throw new Error('Network (mainnet or testnet) must be set in ConcordiumVerificationWebUI constructor');
        }
        ...
        try {
            ...
            localStorage.setItem(ModalConstants.LOCAL_STORAGE_FLAGS.SDK_PROJECT_ID, config.projectId);
            localStorage.setItem(ModalConstants.LOCAL_STORAGE_FLAGS.SDK_NETWORK, network);
            localStorage.setItem(ModalConstants.LOCAL_STORAGE_FLAGS.CONNECTION_MODE, 'sdk-managed');
     ...
     */
    async showModal(modalType: 'landing' | 'scan' | 'processing' | 'returning-user' = 'landing'): Promise<void> {
        switch (modalType) {
            case 'landing':
                await ConcordiumVerificationWebUI.showLandingModal();
                break;
    ...
    async showSuccessState(): Promise<void> {
        const { showSuccessState } = await import('./components/desktop/processing');
    ...
    async closeProcessingModal(): Promise<void> {
        const { hideProcessingModal } = await import('./components/desktop/processing');
        hideProcessingModal();
    }
    ...
    async sendPresentationRequest(presentationRequest: PresentationRequest, sessionTopic?: string): Promise<void> {
        ...
        try {
            ...
            const metadata = {
                description: constructorMetadata?.description || 'Requesting verification',
     ...
     * ```
     */
    async sendRequestToExistingSession(presentationRequest: PresentationRequest, sessionTopic: string): Promise<void> {
